...
export default function MangaDetailPage({ params }: { params: { id: string } }) {
  ...
  const manga = {
    ...
    title: params.id === '1' ? 'One Piece' : params.id === '2' ? 'Naruto' : 'Demon Slayer',
    cover: `/images/mangas/${params.id === '1' ? 'one-piece' : params.id === '2' ? 'naruto' : 'demon-slayer'}.jpg`,
    author: params.id === '1' ? 'Eiichiro Oda' : params.id === '2' ? 'Masashi Kishimoto' : 'Koyoharu Gotouge',
    genres: ['Ação', 'Aventura', 'Fantasia'],
    status: params.id === '1' ? 'Em andamento' : 'Completo',
    rating: params.id === '1' ? 4.8 : params.id === '2' ? 4.7 : 4.9,
    chapters: params.id === '1' ? 1089 : params.id === '2' ? 700 : 205,
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        ...
        <div className="space-y-2">
          {manga.chapters.map((chapter) => (
            <Link 
              href={`/mangas/${manga.id}/chapters/${chapter.number}`} 
              key={chapter.number}
